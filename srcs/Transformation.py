...
def add_image_to_plot(axes: plt.axes, pos_x: int, pos_y: int, title: str,
                      img: np.ndarray):
...
def add_landmarks_to_image(posMarks, img):
...
def add_points_to_img(posBranch, img):
    coordinates = np.column_stack(np.where(posBranch != 0))
    for y, x in coordinates:
        img = cv2.circle(img, (x, y), radius=5,
...
def transformation_task(path: str, plot: bool):
    ...
    a_thresh = pcv.threshold.binary(
        gray_img=a, threshold=120, object_type='dark')

    roi1 = pcv.roi.rectangle(img=a_thresh, x=12, y=12,
                             h=a_thresh.shape[0]-24, w=a_thresh.shape[1]-24)
    ...
    pruned_skel, _, _ = pcv.morphology.prune(
        skel_img=skeleton, size=50, mask=mask_fill)

    img_tips_pts = add_points_to_img(pcv.morphology.find_tips(
        skel_img=pruned_skel, mask=mask_fill, label="default"),
                                     pruned_skel.copy())

    img_branch_pts = add_points_to_img(pcv.morphology.find_branch_pts(
        skel_img=pruned_skel, mask=mask_fill, label="default"),
                                       pruned_skel.copy())

    analyse_img = pcv.analyze.size(
        img=img, labeled_mask=mask_fill, label="default")

    img_pseudolandmarks_x = add_landmarks_to_image(
        pcv.homology.x_axis_pseudolandmarks(img=img, mask=mask_fill),
        img.copy())
    img_pseudolandmarks_y = add_landmarks_to_image(
        pcv.homology.y_axis_pseudolandmarks(img=img, mask=mask_fill),
        img.copy())
    ...
    if plot:
        ...
        add_image_to_plot(
            axes, 2, 0, "Find branch in skeleton", img_branch_pts)
        add_image_to_plot(axes, 2, 1, "Analyse Image", analyse_img)
        add_image_to_plot(axes, 2, 2, "Pseudolandmarks X",
                          img_pseudolandmarks_x)
        add_image_to_plot(axes, 2, 3, "pseudolandmarks Y",
                          img_pseudolandmarks_y)
        plt.show()
        plt.close()
    return pruned_skel, img_tips_pts, img_branch_pts, analyse_img, \
        img_pseudolandmarks_x, img_pseudolandmarks_y
...
def save_transformed_imgs(origin_path: str,
                          to_save_image_list: List[np.ndarray],
                          destination_path: str):
    origin_dir = "/" + origin_path[:origin_path.rfind('/')]
    os.makedirs(destination_path + origin_dir, exist_ok=True)
    suffix = ["_skeleton", "_tips_point", "_branch_point",
              "_analysed_shape", "_pseudolandmarksX", "_pseudolandmarksY"]
...
def transform_img(path, destination=None):
    ...
    save_transformed_imgs(path, modified_img_list, destination)
